...
在读取问卷数据后、生成图表之前，**必须先检测数据中的选项是否有明确的文本标签**。如果数据值仅为数字编码（如 1、2、3）而非带文字描述的选项（如 "A.非常满意"），则**禁止自行猜测标签含义直接生成报告**，必须先向用户确认处理方式。

### 检测方法

读取数据后，扫描每道单选/多选题的数据列：

```python
# 检测某列的值是否为纯数字编码（无文本标签）
def has_text_labels(data, col):
    """返回 True 如果数据值含文本标签（如 'A.非常满意'），False 如果仅为数字编码"""
    for row in data:
        v = str(row.get(col, '')).strip()
        if v and not v.isdigit():
            return True  # 发现非数字值，说明有文本标签
    return False  # 所有值都是数字，说明缺少文本标签
```

### 确认流程

当检测到选项缺少文本标签时，使用 `AskUserQuestion` 工具向用户提供以下三个选项：

```
问题：检测到问卷数据中的选项仅有数字编号（如 1、2、3），缺少文字标签。
你希望如何处理选项标签？

选项1：我提供选项标签（推荐）
  - 用户提供电子版问卷、导出的带标签数据、或直接告知每题选项对照表
  - 生成报告时使用用户提供的真实标签，确保准确性

选项2：按原始数字编号生成
  - 直接使用数字编号（1、2、3...）作为选项标签
  - 报告中的图表标签显示为"选项1"、"选项2"等
  - 适合用户暂时没有问卷原文、但需要快速查看数据分布的场景

选项3：AI 根据题目内容推测标签
  - AI 根据题目标题和上下文推测每个选项的含义
  - 报告中会标注"标签为AI推测，仅供参考"
  - 存在推测错误的风险，需用户自行核实
```
...
| 用户选择 | 处理方式 | 报告标注 |
|----------|----------|----------|
| 提供标签 | 等待用户提供标签对照表后，用真实标签生成报告 | 无需额外标注 |
| 按数字编号 | 使用 `"选项1"、"选项2"` 格式作为标签 | insight-box 中注明"选项标签为原始编号" |
| AI推测 | 根据题目标题推测标签，如满意度题推测为"非常满意/比较满意/不太满意" | **每道题的 section-desc 中必须追加**："⚠️ 以下选项标签为AI推测，可能与实际问卷选项不符，请以原始问卷为准。" |

### ⛔ 严禁行为

**无论何种情况，都不得在未告知用户的情况下自行猜测选项标签并生成报告。** 这会导致报告中出现看似合理但实际错误的数据解读，误导决策。如果用户选择了"AI推测"，必须在报告中明确标注推测属性，让报告读者知晓标签的不确定性。
...
| 序号 | 元素 | 说明 |
|------|------|------|
| ✅ 选项标签确认 | 检测选项是否有文本标签，缺少时必须向用户确认处理方式（见零节） |
...
- **选项标签未确认就生成报告（最严重）**：问卷数据选项仅为数字编码（1、2、3）时，AI 自行猜测标签含义（如把 1 猜成"非常满意"）→ 报告中出现看似合理但实际错误的数据解读，误导决策。**修复**：生成报告前必须按零节流程检测标签是否存在，缺少时必须向用户确认处理方式，不得自行猜测
